...
function createEventWaitingOverlay() {
  ...
  overlay.innerHTML = `
    <div style="
      ...
      border-radius: 24px;
      background: #ffffff;
      box-shadow: 0 18px 45px rgba(32,45,70,.12);
      text-align: center;
    ">
      <div style="
        display: inline-flex;
...
function removeEventWaitingOverlay() {
  const overlay = document.getElementById("eventWaitingOverlay");
...
function checkEventOpenTime() {
  const now = new Date();

  if (now < EVENT_OPEN_TIME) {
    createEventWaitingOverlay();
  } else {
    removeEventWaitingOverlay();
  }
}
...
// 페이지를 계속 열어둔 경우에도 행사 시간이 되면 자동 개방
setInterval(checkEventOpenTime, 1000);
